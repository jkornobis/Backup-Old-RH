...
if(!isset($moisdebut)|| $anneedebut != date("Y") || $anneeactuel != date("Y")){$moisdebut = 1; $jourdebut = 1;}
if(!isset($moisfin) || $anneefin != date("Y") || $anneeactuel != date("Y")){$moisfin = 12; $jourfin = 31;}

if($formation == true){
/*
	$texteformation = '<div id="overlay" class="overlay" onclick="window.location.href=\''.$nompage.'\'"><fieldset class="formation"><legend style="padding-left:20px;padding-right:20px;">
	<b>Formation:</b> <i>"Mon Réalisé"</i></legend>
	<p style="font-size:15px;">
	Bienvenue sur la page <i>"Mon Réalisé"</i> de votre module Personnel.<br/>
	Cette page vous permet de consulter le temps de travail saisi dans OBM. Vous pouvez voir actuellement 3 tableaux:<br/>
	</p>
	<ul style="font-size:16px;">
	<li>Le tableau général qui vous donne jour par jour et mois par mois votre temps de travail ainsi que vos congés et autres événements spéciaux.</li><br/>
	<li>Le tableau récapitulatif mois par mois de vos congés, RC et RTT ainsi que les calculs de vos soldes du mois et de l\'année.</li><br/>
	<li>Le tableau récapitulatif annuel de vos heures réalisées ainsi que vos congés.</li>
	</ul>
	</fieldset>
</div>
';
...
<span style="font-size:17px;"> Date et heure: '.date("d/m/Y H\Hi").'</span></h2>'.$texteformation;
...
while( $ligne = $resultat->fetch()){
	...
	if($h >= "18" || $h < "6"){
		$warning .= '<p style="color:#A00;font-size:14px;font-weight:bold;"><i>"'.$ligne->event_title.'"</i> du <b>'.$jour.'/'.$mois.'/'.$annee.'</b> à <b>'.($h+1).'H'.$m.'min</b> ne fais pas partie des horaires habituelles de travail (mais tout de même comptabilisé).<br/> </p>';
	}
	if($catcode < "900"){
		if($ligne->eventlink_state != "ACCEPTED"){
			$warning .= '<p style="color:#A00;font-size:14px;font-weight:bold;">La réunion <i>"'.$ligne->event_title.'"</i> du <b>'.$jour.'/'.$mois.'/'.$annee.'</b> à <b>'.($h+1).'H'.$m.'min</b> doit être accepté pour être comptabilisé ('.($ligne->event_duration/3600).' heures dans votre réalisé.)</p>';
//' <a href="controle.php?suppr='.$eventid.'&uti='.$utilisateur.'" onclick="return(confirm(\'Etes-vous sûr de vouloir Supprimer cette événement ?\'));">Supprimer</a></p>';
		}else{
			...
			if(date("l" , $timestamp[$jcomp]) == 'Sunday' || date("l", $timestamp[$jcomp]) == 'Saturday' ){
				...
				if (isset($rh[$mcomp][$jcomp])){
					switch($rh[$mcomp][$jcomp]){
						...
						//		Congés Ancienneté
						case 'Canc':
							echo '<td style="background:orange;color:#000;font-weight:bold;text-align:center;" title="'.$ligne->event_description.'">C.Anc</td>';		
						break;
						case 'Cancv':
							echo '<td style="background:#FF3;color:#000;font-weight:bold;text-align:center;" '.$ligne->event_description.'>C.Anc</td>';		
						break;
						case 'Cancr':
							echo '<td style="background:#A00;color:#000;font-weight:bold;text-align:center;" '.$ligne->event_description.'>C.Anc</td>';		
						break;
...
if (isset($warning)){
	echo '<div id="warning"><fieldset style="color:#A00;min-width:1000px;"><legend style="color:#A00;font-size:28px;">En Travaux:</legend>
	<h2 style="color:#A00;text-decoration:underline;">Cas particuliers constatées:</h2>';
	echo $warning.'</fieldset></div><br/>';
}
